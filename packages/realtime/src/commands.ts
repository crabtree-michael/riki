 ...
 *
 * Nothing else in this product parses natural language into an action. There used to be a second
 * path — the model turning speech into tool calls — and ADR-0023 deleted it, which leaves these
 * four as the whole of what a spoken phrase can *do*.
 *
 * **How much this earns changed with that deletion, and upward.** voice-input §6.2 was honest that
 * under push-to-talk it earned very little: the player must hold the trigger to be heard at all,
 * and holding the trigger during Speaking *is* barge-in. But Riki now speaks *unprompted*
 * (coaching-architecture.md §7.1), so the player is not holding anything when it does — and
 * `quiet-mode`, "only when I ask", is the off switch for the primary path. It must work with the
 * model down.
 *
