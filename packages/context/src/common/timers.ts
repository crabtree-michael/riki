/**
 * The one impure thing this package does, behind an interface so it is not impure in a test.
 *
 * `Clock` (`./types.ts`) answers *what time is it*; this answers *wake me later*. They are separate
 * because a caller often needs both and a test needs to control both without waiting real time to
 * watch a deadline fire — `ManualTimers` in `../testing/` is what makes every deadline test run in
 * microseconds and never flake.
 *
 * The one consumer is Tier 1 preamble enrichment, whose 3-second deadline is a race rather than a
 * timeout (`../preamble/assemble.ts`). Nothing on the per-turn path has a deadline at all, which is
 * the point of docs/design/coaching-architecture.md §3.1.
 */

export interface Timers {
  /** Returns a cancel function. Calling it after the timer fired is a no-op. */
  after(ms: number, fn: () => void): () => void;
}

export const systemTimers: Timers = {
  after(ms: number, fn: () => void): () => void {
    // `unref` where it exists: a pending deadline must never be the reason the process stays up.
    const handle: ReturnType<typeof setTimeout> = setTimeout(fn, Math.max(0, ms));
    if (typeof handle === 'object' && 'unref' in handle) handle.unref();
    return () => {
      clearTimeout(handle);
    };
  },
};
