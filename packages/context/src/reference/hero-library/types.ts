 ...
 *
 * The library is *reference* data in the sense agent-command-execution-architecture.md §5.3 already
 * means — the data that is not about this match. It holds nothing observed, nothing timestamped and
 * nothing per-player, which is why none of it is `Observed<T>`: there is no age to render because
 * there is nothing here that was seen.
 *
 ...
 *
 * Closed, and small on purpose: the values are shown to the model as a JSON Schema enum inside the
 * frozen manifest, so each one is a permanent cost on the cached prefix (§8.1). With no free-text
 * argument to fall back on (ADR-0023), this enum is the *only* way the agent narrows a request, so
 * every value has to earn its place and none may be a synonym of another.
 *
 ...
 * paragraph to be compressed. The 120-token result ceiling of the `reference` effect class fits
 * roughly six of them, which is the constraint that decided the shape (hero-library.md §3).
 */
...
/**
 * Two closed values, and nothing else — no free text, here or on the command that fills it.
 *
 * ADR-0023 closes the tool vocabulary. Its sharpest argument is about egress and does not apply to
 * a local reader, but the decision is written as an absolute and the shape it asks for turned out
 * to be the better one anyway: `topic` filters six one-line notes more precisely than term overlap
 * ever did.
 */
...
export interface HeroLibraryResult {
  ...
  readonly patch: string;
  readonly positions: readonly Position[];
  /** Ranked, already capped. The renderer still applies the token budget on top. */
  readonly notes: readonly HeroNote[];
}
