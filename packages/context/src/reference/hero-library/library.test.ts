...
import { createStaticHeroLibrary } from './index.js';
...
describe('searchHeroLibrary', () => {
  ...
  it('takes no free text at all — narrowing is the only thing a caller can do', () => {
    ...
    // order — which is what the search now does in one step — and it cost ~33 manifest tokens on
...
describe('createStaticHeroLibrary', () => {
  it('answers a covered hero with the notes', async () => {
    const outcome = await createStaticHeroLibrary().heroLibrary({ hero: hero('undying') });
    expect(outcome.ok).toBe(true);
  });

  it('answers an uncovered hero with a speakable sentence, not a hero id', async () => {
    const outcome = await createStaticHeroLibrary().heroLibrary({ hero: hero('skeleton_king_x') });

    expect(outcome.ok).toBe(false);
    if (outcome.ok) return;
    expect(outcome.failure.code).toBe('unavailable');
    expect(outcome.failure.speakable).toBe("I don't have notes on that hero.");
    // The canonical id is diagnostic and stays out of what the model is handed (§13's egress rule).
    expect(outcome.failure.speakable).not.toContain('_');
    expect(outcome.failure.detail).toContain('skeleton_king_x');
  });

  it('takes an injected library, which is what a test with an uncovered hero needs', async () => {
    const empty: HeroLibrary = { patch: 'x', authored: 'y', entries: [] };
    const outcome = await createStaticHeroLibrary(empty).heroLibrary({ hero: hero('spectre') });
    expect(outcome.ok).toBe(false);
  });
});
