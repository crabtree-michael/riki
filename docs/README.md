...
| [0048](adr/0048-world-at-takes-a-clock-or-an-offset-and-they-are-different-axes.md) | `world_at` takes a clock or an offset, and they are different axes | Accepted — `clock` seeks on match time, `seconds_ago` on wall time, exactly one of them, never converted into each other. Also moves the replay bound from one keyframe interval to the delta window, so `recently_lost` is not a false empty. Settles conversational-architecture open question 2 |
| [0049](adr/0049-a-failed-tool-call-is-an-unknown-not-a-silence.md) | A failed tool call is an `unknown`, not a silence | Accepted — every way a call fails on our side answers `{unknown: reason}`, valid for all five tools; the manifest is sent only when a dispatcher is injected; the continuation `response.create` is the turn controller's and a cancelled turn does not get one |
| [0050](adr/0050-riki-is-a-coach-that-does-not-interrupt.md) | Riki is a coach that does not interrupt | Accepted — refines [0042](adr/0042-riki-answers-questions-instead-of-deciding-when-to-speak.md): the prompt said *"You are not a coach"* and a player who asked to be coached was refused by the prefix. Identity stated positively; no-unprompted-speech dropped from the prompt because push-to-talk already guarantees it |
| [0051](adr/0051-a-tool-call-is-a-request-and-the-renderer-holds-its-deadline.md) | A tool call is a request across the preload bridge, and the renderer holds its deadline | Accepted — `voice.tool.call`/`voice.tool.result` correlated by a renderer-allocated `callId`, `VoiceSessionOpen.tools` carrying ADR-0049's coupling across the boundary, a 2 s renderer-side deadline that answers `{unknown}`, and a result routed past the directive queue. Discharges the message ADR-0049 records as owed |
