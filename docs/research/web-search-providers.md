...
[ADR-0027](../adr/0027-the-hero-library-is-static.md) makes the hero library **static, hand-authored
...
A patch-length cache — the shortest useful life for this content — is not transient storage. There
is no reading of it that survives this clause.
...
moment that stops being good enough. A separate question — whether we may commit a fixture corpus of
real responses to the repo — is **not** covered by any of this, because a committed corpus is
redistribution by any reading. See §6.
...
`include_domains` would implement a source allowlist server-side, which is where it belongs.

### 4.2 Measured latency confirms the design's central claim

The live design's central claim was that a search cannot fit inside the latency budget an advice
path has. The 400 ms figure below was the deleted command pipeline's `reference` deadline; the brief
that replaced it is budgeted at **under 5 ms**, so the conclusion only got stronger:
...
Take the vendor entirely at its word and the typical case is 210 ms against that 400 ms budget, with
...
**Confirmed, and measured rather than assumed.** A search cannot happen inside a turn — not against
the budget that existed when this was researched, and by three orders of magnitude not against the
one that exists now.
...
The live design bounded worst-case traffic at **~375 requests per patch** (25 heroes × 5 topics,
